...
- **`server.py`** — MCP protocol over stdin/stdout for Claude Desktop. Tools: `list_portfolios`, `get_portfolio_summary`, `get_portfolio_positions`, `get_portfolio_operations`, `compare_portfolios`, `get_consolidated_summary`, `get_app_settings`, `get_position_with_quote`, `get_dividends_summary`, `get_concentration_analysis`, `get_portfolio_performance`, `get_fixed_income_summary`, `get_portfolio_alerts`.
- **`http_api.py`** — FastAPI REST API for the frontend. Pydantic response models use **camelCase** field names; analytical endpoints proxied from MCP tools return the tool dicts verbatim (snake_case). Key routes: `/api/portfolios`, `/api/portfolios/{id}/summary`, `/api/portfolios/{id}/positions`, `/api/portfolios/{id}/operations`, `/api/portfolios/{id}/fixed-income`, `/api/portfolios/{id}/previdencia`, `/api/quotes/refresh`, `/api/settings`, `/api/portfolios/{id}/positions-with-quote`, `/api/portfolios/{id}/dividends-summary`, `/api/portfolios/{id}/concentration`, `/api/portfolios/{id}/performance`, `/api/portfolios/{id}/fixed-income-summary`, `/api/portfolios/{id}/alerts`.
