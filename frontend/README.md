...
> `src/mocks/data.ts`. Quando o backend FastAPI for adicionado em uma PR
> seguinte, os hooks em `src/lib/api.ts` passam a consumir os endpoints
> reais sem alterações nas páginas.
...
3. Substituir os imports de `@/mocks/data` pelos hooks TanStack Query
   chamando `apiFetch`.
