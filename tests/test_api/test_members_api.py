...
def _client(tmp_path: Path) -> TestClient:
    ...
    m_repo.upsert(Member(id="bob", name="Bob", email="bob@example.com"))
...
def test_get_member(tmp_path: Path) -> None:
    client = _client(tmp_path)
    response = client.get("/api/members/bob")
    assert response.status_code == 200
    payload = response.json()
    assert payload["id"] == "bob"
...
def test_create_member_duplicate(tmp_path: Path) -> None:
    client = _client(tmp_path)
    response = client.post("/api/members", json={"id": "bob", "name": "X"})
    assert response.status_code == 422


def test_patch_member_changes_name(tmp_path: Path) -> None:
    client = _client(tmp_path)
    response = client.patch("/api/members/alice", json={"name": "Robert"})
    assert response.status_code == 200
...
def test_list_portfolios_filter_by_owner(tmp_path: Path) -> None:
    client = _client(tmp_path)
    response = client.get("/api/portfolios?ownerId=bob")
    assert response.status_code == 200
    portfolios = response.json()
    assert all(p["ownerId"] == "bob" for p in portfolios)
...
def test_update_portfolio_changes_owner(tmp_path: Path) -> None:
    ...
    response = client.put(
        "/api/portfolios/rv",
        json={"name": "Renda Variável", "ownerId": "alice"},
    )
    assert response.status_code == 200
    payload = response.json()
    assert payload["name"] == "Renda Variável"
    assert payload["ownerId"] == "alice"
